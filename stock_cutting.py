    ...
    ** Nearly Pure Python ** Implementation of the 
    ...
    for p in range(len(ptp)):
        ...
        if packed_in is not None:
            ...
            Result = [ptp[p][2], {'part': ptp[p][0], 'x': int(Cr_EPs[k][L-1][0]) , 'y': int(Cr_EPs[k][L-1][1] + (5+Bin_size[1])*packed_in)}]
    ...
    for i in range(len(SH)):
        y = int((5+ Bin_size[1])*i)
        x = int(0)

        entry = {'sheet': i, 'x': x, 'y':y}
        Sheet_coord.append(entry)
    print('sheetCoords', Sheet_coord)
    ...
    list_sheetCoords = list(Sheet_coord)

    '''for i in range(len(PARTS)):
        print(output[i])'''
    
    OUTPUT = json.dumps({"results":{'parts': list_output,
                'sheets': list_sheetCoords}})
    print('OUTPUT', OUTPUT)
    return OUTPUT